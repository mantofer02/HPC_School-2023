...
cout << "num_elem = " << num_elem << " my_id = " << my_id << endl;

if((N%num_procs) == 0 ){
MPI_Scatter(array,num_elem, MPI_INT,array_recv,num_elem, MPI_INT, 0, MPI_COMM_WORLD);

for(i=0;i<num_elem;i++){
cout << "my_id = " << my_id <<", Received elements : " << array_recv[i] << endl;
...
if(my_id == 0){
cout << "N is multiple of num_procs, mod(N,num_procs)= " << N%num_procs << endl;
for(i=0;i<N;i++){
cout << "i = " << i << "; Array_final : " << array_final[i] << endl;
}
}
}
